...
function isShellPageUrl(url) {
  return url.startsWith(SHELL_PAGE_PREFIX);
}
...
async function inspectRenderedPage(win) {
  ...
  return win.webContents.executeJavaScript(`
    (() => {
      ...
      const bodyChildren = document.body ? document.body.childElementCount : -1;
      return {
...
async function verifyMainPageRendered(win, token, reason) {
  ...
  try {
    ...
    showShellError(win, `The app page loaded without visible content after ${reason}.`);
  } catch (error) {
    showShellError(win, error?.message || `The app could not be inspected after ${reason}.`);
  }
}
...
function createWindow() {
  ...
  win.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedUrl, isMainFrame) => {
    ...
    if (isInternalWrapperUrl(failedUrl)) {
      showShellError(win, `${errorDescription || "Load failed"} (${errorCode})`);
    }
  });
