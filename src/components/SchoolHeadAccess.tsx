...
import { Building2, ChartNoAxesCombined, GraduationCap, LogIn, ShieldCheck, Users } from 'lucide-react';
import { Link } from 'react-router-dom';
import { auth, googleProvider, microsoftProvider } from '../firebase';
import SchoolHeadPortal from './portals/SchoolHeadPortal';
import { bindNativeGoogleSignInBridge, isNativeIosGoogleWrapper, requestNativeGoogleSignIn } from '../lib/native-ios-google-auth';

interface SchoolHeadAccessProps {
  user: User | null;
}

function SchoolHeadLoginPanel() {
  ...
  const handleLogin = async (provider: AuthProvider) => {
    ...
    try {
      ...
      setAuthError(error?.message || 'Principal sign-in failed.');
    } finally {
      setIsLoggingIn(false);
    }
  };
      ...
      <div className="mx-auto grid min-h-screen max-w-7xl gap-10 px-6 py-8 lg:grid-cols-[1fr_440px] lg:items-center">
        <section className="space-y-8">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-500 shadow-lg shadow-emerald-950/30">
          ...
          <div className="max-w-3xl">
            <p className="text-xs font-black uppercase tracking-[0.32em] text-emerald-300">School command centre</p>
            <h2 className="mt-5 text-5xl font-black tracking-tight sm:text-6xl">
              See grade trends, teacher oversight, and curriculum coverage in one place.
            </h2>
            <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-zinc-300">
              Principals sign in here to review school-wide performance, monitor teacher reliability, and track curriculum delivery across year levels.
            </p>
          </div>

          <div className="grid max-w-3xl gap-4 sm:grid-cols-3">
            {[
              { label: 'Teacher tracking', icon: Users },
              { label: 'Grade trends', icon: ChartNoAxesCombined },
              { label: 'Curriculum view', icon: Building2 },
            ].map((item) => {
        ...
        <section className="rounded-[32px] border border-white/10 bg-white p-8 text-zinc-950 shadow-2xl shadow-black/30">
          <div className="mb-7 flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-500 text-white">
            <ShieldCheck size={28} />
          </div>
          <h2 className="text-3xl font-black tracking-tight">Principal sign in</h2>
          <p className="mt-3 text-sm font-semibold leading-6 text-zinc-500">
            Use a school Google or Microsoft account to open the principal dashboard.
          </p>
          ...
          <div className="mt-8 flex items-center gap-3 rounded-3xl bg-zinc-50 p-4">
            ...
            <p className="text-xs font-bold leading-5 text-zinc-500">
              This portal is separate from student and teacher access so school leaders land directly in the dashboard.
            </p>
          </div>
...
export default function SchoolHeadAccess({ user }: SchoolHeadAccessProps) {
  if (!user) {
    return <SchoolHeadLoginPanel />;
  }

  return <SchoolHeadPortal />;
}
